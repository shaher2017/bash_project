...
function insert {
  ...
  for ((i = 2; i <= colsNum; i++)); do
    ...
    if [[ $colType == "int" ]]; then
      while ! [[ $data =~ ^[0-9]*$ ]]; do
        echo -e "Invalid input for data type $colType. Please enter an integer value."
        echo -e "$colName ($colType) = \c"
        read data
      done
    elif [[ $colType == "str" ]]; then
      # No need to validate string data type, it can accept any input.
      :
    else
      echo "Unsupported data type: $colType"
      tablesMenu
    fi
    ...
    if [[ $colKey == "PK" ]]; then
      while true; do
        if grep -q "^$data$sep" "$tableName"; then
          echo -e "Invalid input for Primary Key $colName. Value already exists in the table."
        else
          break
        fi
        echo -e "$colName ($colType) = \c"
  ...
  then
    ...
    then
      ...
      then
        ...
        dataType=$(awk -v col="$setField" 'BEGIN{FS="|"}{if ($1 == col) print $2}' .$tName)

        if [[ "$dataType" == "int" && ! "$newValue" =~ ^[0-9]+$ ]]; then
          echo "Invalid data: $newValue. Data type must be int for column $setField."
          tablesMenu
          return
        elif [[ "$dataType" == "str" && "$newValue" =~ [[:space:]] ]]; then
          echo "Invalid data: $newValue. Data type must be str for column $setField and should not contain spaces."
          tablesMenu
          return
        fi
